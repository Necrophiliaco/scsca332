...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton('📢 ɢᴜʀᴜʜ 📢', url=f'https://t.me/{SUPPORT_CHAT}')
            ],[
            InlineKeyboardButton('ℹ️ ʏᴏʀᴅᴀᴍ ℹ️', url=f"https://t.me/{temp.U_NAME}?start=help")
        ]]
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            return
        btn = [[InlineKeyboardButton("Kᴀɴᴀʟɢᴀ ǫᴏ'ꜱʜɪʟɪꜱʜ ✨", url=invite_link.invite_link)]]
        if message.command[1] != "subscribe":
            try:
                kk, file_id = message.command[1].split("_", 1)
                pre = 'checksubp' if kk == 'filep' else 'checksub' 
                btn.append([InlineKeyboardButton("⟳ Tᴇᴋꜱʜɪʀɪꜱʜ", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("⟳ Tᴇᴋꜱʜɪʀɪꜱʜ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
